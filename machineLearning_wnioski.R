...
#Korelacja Spearmana zmiennych numerycznych ze zmienna celu
abs(cor(dane[,num], as.numeric(dane$Wynik), method = "spearman"))
...
dim(zbior_testowy)
table(zbior_testowy$Wynik)
dim(zbior_treningowy)
table(zbior_treningowy$Wynik)
...
#najlepszy podzbior dla 9 zmiennych
#zmienne: Poz_hist_kred, Score_kredytowy, Lata_pracy, Przychody
# Umowa_o_prace, Wykształcenie, Pochodzenie, Stan_cywilny, Bank

model2 <- train(Wynik~Poz_hist_kred+Score_kredytowy+Przychody+Lata_pracy+
                  Umowa_o_prace+Pochodzenie.ff+Wykształcenie.k+
                  Wykształcenie.ff+Bank.g+Bank.p+Stan_cywilny.u+Stan_cywilny.y+
                  Wykształcenie.aa+Obywatelstwo.s+Obywatelstwo.g,
...
#   Poz_hist_kred, Score_kredytowy, Przychody, Lata_pracy,
# Umowa_o_prace, Pochodzenie, Wykształcenie, Bank, Stan_Cywilny, Obywatelstwo
